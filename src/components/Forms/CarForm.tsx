...
<p><strong>Buyer Due Diligence</strong></p>

<ul>
  <li><strong>ALL PROSPECTIVE BUYERS ARE STRONGLY ENCOURAGED</strong> to conduct thorough due diligence prior to purchase.</li>
</ul>

<p>This includes:</p>

<ul>
  <li><strong>In-Person Inspection:</strong> Arranging a personal inspection of the vehicle in person.</li>
  <li><strong>Professional Consultation:</strong> Consulting with a registered vehicle repairer or independent inspector to verify the vehicle's details and condition.</li>
  <li><strong>Virtual Inspection:</strong> Utilising our detailed website photographs and, upon arrangement, we can conduct a live video connection (via WhatsApp, FaceTime, FaceBook Messenger, or Zoom) to view the car up close.</li>
</ul>
`.trim();
...
export default function CarForm({
  ...
  const [existingImages, setExistingImages] = useState<
    { key: string; orientation: "portrait" | "landscape" }[]
  >(
    initialData?.imageKeys?.map((key) => ({
      key: key.key || key,
      orientation: key.orientation || "landscape",
    })) || []
  );
  const [existingVideos, setExistingVideos] = useState<string[]>(
    initialData?.videoKeys || []
  ...
  const {
    ...
    defaultValues: {
      ...
      make: initialData?.make || "",
      description:
        initialData?.description || (isNew ? DEFAULT_DESCRIPTION : ""),
      price: initialData?.price || 0,
      factoryOptions: initialData?.factoryOptions || [],
      highlights: initialData?.highlights || [],
      keyFeatures: initialData?.keyFeatures || [
  ...
  const clearDraft = async () => {
    try {
      reset({
        title: "",
        make: "",
        description: isNew ? DEFAULT_DESCRIPTION : "",
        price: 0,
        factoryOptions: [],
        highlights: [],
        keyFeatures: [],
  ...
  useEffect(() => {
    ...
    if (initialData?.imageKeys) {
      setExistingImages(
        initialData.imageKeys.map((ik) => ({
          key: typeof ik === "string" ? ik : ik.key,
          orientation: typeof ik === "string" ? "landscape" : ik.orientation,
        }))
      );
      const initialOrientations = initialData.imageKeys.reduce(
        (acc, ik, index) => {
          const orientation =
            typeof ik === "string"
              ? "landscape"
              : ik.orientation || "landscape";
          return { ...acc, [index]: orientation };
        },
        {}
      );
  ...
  // Set default description on new car form mount
  useEffect(() => {
    if (isNew && !initialData && !getValues("description")) {
      setValue("description", DEFAULT_DESCRIPTION, { shouldValidate: true });
    }
  }, [isNew, initialData, setValue, getValues]);
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        newPreviews.push(url);
        newIds.push(id);
        newOrientations[existingImages.length + images.length + i] =
          "landscape";
      } catch (err) {
        console.error("Failed to save image to IndexedDB", err);
      }
  ...
  const handleFormSubmit = async (formData: CreateCarRequest) => {
    ...
    try {
      ...
      const allExistingImagesWithOrientation = existingImages.map(
        (img, index) => ({
          key: img.key,
          orientation:
            imageOrientations[index] || img.orientation || "landscape",
        })
      );
      ...
      const newImagesWithOrientation = images.map((_, index) => ({
        key: `new-image-${index}`,
        orientation:
          imageOrientations[existingImages.length + index] || "landscape",
      }));

      const allImageKeys = [
        ...allExistingImagesWithOrientation,
        ...newImagesWithOrientation,
      ];

...
//   const handleSubmit = async (data: UpdateCarRequest, images?: File[]) => {
//     try {
//       if (!id) return;
//       await carsService.updateCar(id, data, images);
...
export default function EditCarPage() {
  ...
  const handleSubmit = async (data: UpdateCarRequest, images: File[], existingImages: string[]) => {
    try {
      if (!id) return;
      await carsService.updateCar(id, { ...data, images: existingImages }, images);
      toast.success('Car updated successfully!');
      navigate('/dashboard');
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Failed to update car');
